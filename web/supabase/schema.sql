...
create table if not exists public.orders (
  ...
  notes text,
  design_ref text,
  bigseller_line_items jsonb not null default '[]'::jsonb,
  job_type_id uuid references public.job_types(id) on delete set null,
  jersey_line_prices jsonb not null default '{}'::jsonb,
  return_status text check (return_status in ('returning','returned')),
  return_reason text,
  return_inventory_type text check (return_inventory_type in ('inventory','ready_made')),
  return_inventory_ref jsonb,
