...
type CartItem = {
  _key: string;
  product_name: string;
  product_source: "inventory" | "custom";
  inventory_id?: string | null;
  inventory_stock?: number | null; // current stock for display
  unit?: string | null;
  quantity: number;
  unit_price: number;
};
...
export function PosClient({ inventoryItems, financeAccounts, viewerRole }: Props) {
  ...
  // Custom product form (add item not in inventory)
  ...
  function addInventoryItem(item: InventoryItem) {
    setCart((prev) => {
      const existing = prev.find(
        (ci) => ci.product_source === "inventory" && ci.inventory_id === item.id,
      );
      if (existing) {
        return prev.map((ci) =>
          ci._key === existing._key ? { ...ci, quantity: ci.quantity + 1 } : ci,
        );
      }
      return [
        ...prev,
  ...
  function updateCartItem(key: string, field: "quantity" | "unit_price", value: number) {
    setCart((prev) =>
      prev.map((ci) => (ci._key === key ? { ...ci, [field]: Math.max(0, value) } : ci)),
    );
  }
  ...
  async function completeSale() {
    ...
    try {
      ...
      const lineItems = cart.map((ci, idx) => ({
        order_id: order.id,
      ...
      const { error: itemsErr } = await supabase.from("pos_order_items").insert(lineItems);
      if (itemsErr) {
      ...
      // 3. Deduct inventory quantity for inventory-sourced items
        ...
        if (ci.product_source === "inventory" && ci.inventory_id) {
          ...
          if (inv != null) {
            ...
            await supabase
              ...
              .eq("id", ci.inventory_id);
          }
        }
      }

      // 4. Record payment to finance account if selected
      if (paymentAccountId) {
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 px-4 py-16 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Order #{successOrderNo} has been recorded.</p>
        </div>
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-4 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          ...
          {showCustomForm && (
            <div className="mb-3 rounded-lg border border-border bg-background p-3 shadow-sm">
              ...
              <button
                className="mt-2 w-full rounded-md bg-primary py-2 text-xs font-medium text-primary-foreground hover:bg-primary/90"
                    ...
                    className={
                      ...
                      (inCart
                        ...
                        : outOfStock
                          ...
                          : "border-border bg-background hover:border-primary/40 hover:bg-accent/50 hover:shadow-sm")
                    }
                  ...
                  >
                    ...
                    <div className="mt-auto flex w-full items-center justify-between pt-1">
                      ...
                      >
                        {outOfStock
                          ? "Out of stock"
                          : `${item.quantity ?? 0}${item.unit ? " " + item.unit : ""} left`}
                      </span>
                    </div>
                  </button>
                );
      ...
      <div className="flex w-[45%] flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
              ...
              {cart.map((ci) => (
                <div key={ci._key} className="flex items-center gap-2 px-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {ci.product_source === "inventory" && ci.inventory_stock != null && (
                      <p className="text-[10px] text-muted-foreground">
                        Stock: {ci.inventory_stock}
                        {ci.unit ? " " + ci.unit : ""}
                      </p>
                    )}
                    {ci.product_source === "custom" && (
                  ...
                  <div className="shrink-0">
                    <input
                      ...
                      min={0}
                      step="0.01"
                      className="h-7 w-24 rounded border border-border bg-transparent px-2 text-right text-xs font-mono outline-none focus:border-primary/60"
                      value={ci.unit_price}
                      onChange={(e) =>
                        updateCartItem(ci._key, "unit_price", parseFloat(e.target.value) || 0)
                      }
                      title="Unit price"
                    />
                  </div>
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    <button
                      className="flex h-6 w-6 items-center justify-center rounded border border-border bg-muted text-muted-foreground hover:bg-accent hover:text-foreground"
                      onClick={() => updateCartItem(ci._key, "quantity", ci.quantity - 1)}
                    >
                    ...
                    <input
                      ...
                      value={ci.quantity}
                      onChange={(e) =>
                        updateCartItem(ci._key, "quantity", parseFloat(e.target.value) || 1)
                      }
                    />
                    <button
                      className="flex h-6 w-6 items-center justify-center rounded border border-border bg-muted text-muted-foreground hover:bg-accent hover:text-foreground"
                  ...
                  <button
                    ...
                    onClick={() => removeCartItem(ci._key)}
                  >
        ...
        <div className="border-t border-border bg-muted/20 px-4 py-4">
          {/* Notes */}
          <textarea
            className="mb-3 h-16 w-full resize-none rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/30"
          ...
          <div className="mb-4 space-y-1 rounded-lg bg-background px-4 py-3 text-sm">
            <div className="flex justify-between">
              <span className="text-muted-foreground">
                Items ({cart.reduce((s, ci) => s + ci.quantity, 0)})
              </span>
          ...
          {/* Complete Sale */}
          <button
            className="flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-primary text-sm font-semibold text-primary-foreground shadow transition-all hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
